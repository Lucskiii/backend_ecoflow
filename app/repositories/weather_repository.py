...
class WeatherRepository:
    def __init__(self, db: Session):
        ...
        self.quality_flag_code_col = self._column(self.quality_flag_table, "quality_flag", "code")
        self.quality_flag_desc_col = self._column(self.quality_flag_table, "description")
    ...
    def get_or_create_weather_location(
        ...
        existing = self.db.execute(
            select(self.weather_loc_id_col).where(
    ...
    def upsert_weather_observations(self, rows: list[dict[str, Any]]) -> int:
        ...
        if dialect_name == "mysql":
            ...
            update_map = {
                ...
                for column in ["temp_c", "wind_ms", "ghi_wm2", "cloud_pct", "quality_flag", "ingestion_batch_id"]
                if column in self.weather_observation_table.c
            }
            result = self.db.execute(stmt.on_duplicate_key_update(**update_map))
